...
In `tidy.txt`, the average of each variable for each activity and for each subject is given. Since the features were unedited, apart from taking a simple mean across each subject and eacg activity, it is highly recommended that this codebook be read in conjunction with the original codebook supplied with the dataset. The features retained are as follows:
